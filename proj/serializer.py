...
class GovernmentProjectSerializer(serializers.ModelSerializer):
    ...
    def get_traders(self, obj):
        ...
        if qs.exists():
            return GovernmentProjectTraderSerializer(qs, many=True).data
        return None
...
class GovernmentProjectDetailSerializer(serializers.ModelSerializer):
    ...
    def get_infos(self, obj):
        qs = obj.govproj_infos.filter(is_deleted=False)
        if qs.exists():
            return GovernmentProjectInfoSerializer(qs, many=True).data
        return None

    def get_historycases(self, obj):
        qs = obj.govproj_historycases.filter(is_deleted=False)
        if qs.exists():
...
class GovernmentProjectTraderSerializer(serializers.ModelSerializer):
    trader = UserNameSerializer()
    class Meta:
        model = GovernmentProjectTrader
        exclude = ('deleteduser', 'datasource', 'is_deleted', 'deletedtime')


class GovernmentProjectInfoCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = GovernmentProjectInfo
        fields = '__all__'


class GovernmentProjectInfoSerializer(serializers.ModelSerializer):
    ...
    def get_attachments(self, obj):
        qs = obj.govprojinfo_attachments.filter(is_deleted=False)
        if qs.exists():
            return GovernmentProjectInfoAttachmentCreateSerializer(qs, many=True).data
        return None


class GovernmentProjectInfoAttachmentCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = GovernmentProjectInfoAttachment
        fields = '__all__'

class GovernmentProjectInfoAttachmentSerializer(serializers.ModelSerializer):
...
class GovernmentProjectHistoryCaseCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = GovernmentProjectHistoryCase
        fields = '__all__'

class GovernmentProjectHistoryCaseSerializer(serializers.ModelSerializer):
