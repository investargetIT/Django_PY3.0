...
class projectTaxiRecord(MyModel):
    proj = MyForeignKey(project, related_name='project_TaxiRecords')
    orderNumber = models.CharField(max_length=32, null=True, help_text='订单号')
    orderType = models.CharField(max_length=64, null=True, help_text='订单类型')
    ...
    deleteduser = MyForeignKey(MyUser, blank=True, null=True, related_name='userdelete_projTaxiRecords')
    createuser = MyForeignKey(MyUser, blank=True, null=True, related_name='usercreate_projTaxiRecords')

    class Meta:
        db_table = "project_TaxiRecord"

    def save(self, *args, **kwargs):
        if projectTaxiRecord.objects.exclude(pk=self.pk).filter(orderNumber=self.orderNumber, is_deleted=False).exists():
            raise InvestError(4010, msg='订单已存在')
        return super(projectTaxiRecord, self).save(*args, **kwargs)
