            ...
            if checkresponse['is_end']:
                if os.path.exists(file_path):
                    ...
                    QiNiuFileUploadRecord(filename=data['filename'], filesize=data['totalSize'], fileMD5=data['md5'],
                                          ...
                                          convertToPDF=changeToPdf, convertKey=outputFileKey).save()
                    uploadFileToQiniu()
...
def uploadFileToQiniu():
    ...
    if not os.path.exists(markfilepath):
        ...
        if quesys.first().starttime < (datetime.datetime.now() - datetime.timedelta(minutes=10)):
            quesys.first().update(status=1, success1=False, success2=False, info1=None, info2=None, msg=None, starttime=None, endtime=None)
